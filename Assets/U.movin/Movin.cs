...
public class Movin
{
    ...
    public Movin(Transform parent, string path, int sort = 0, float scale = 1f, float strokeWidth = 0.5f,
        bool loop = true, float quality = 0.4f, float imgScale = 1f)
    {
        ...
        MovinInit(path, sort, scale, strokeWidth, loop, quality, imgScale);
    ...
    private void MovinInit(string path, int sort = 0, float scale = 1f, float strokeWidth = 0.5f, bool loop = true,
        float quality = 0.4f, float imgScale = 1f){
        
        directory = path.IndexOfAny(new char[] { '/', '\\' }) == -1 
            ? "" 
        ...
        for (int i = 0; i < layers.Length; i++) {
            ...
            {
                parent = layersByIndex[p].transform.GetChild(0);
            }
            else
            {
                parent = layersByIndex[p].transform;
            }
