...
namespace u.movin
{
    public struct BodymovinContent
    {
        ...
        public static BodymovinContent init(string jsonPath)
        {
            ...
            ParseAssets(ref content, data);
            ParseLayers(ref content, data);
            return content;
        }

        private static void ParseAssets(ref BodymovinContent b, JSONNode n)
        {
            b.assets = new BodymovinAssets[n["assets"].Count];
            for (int i = 0; i < n["assets"].Count; i++)
            {
                JSONNode a = n["assets"][i];
                b.assets[i] = new BodymovinAssets()
                {
                    id = a["id"],
                    w = a["w"],
                    h = a["h"],
